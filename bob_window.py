...
class BobWindow:
    def __init__(self, master, main_window):
        ...
        self.bob_window = tk.Toplevel(master)
        self.bob_window.title("Окно Bob")
        self.bob_window.geometry("800x300")  # Увеличил ширину окна

        # Заголовок Боб
        bob_label = tk.Label(self.bob_window, text="Боб", font=("Arial", 16, "bold"))
        bob_label.grid(row=0, column=0, columnspan=3, pady=10)

        # Поле для открытого ключа
        open_key_label = tk.Label(self.bob_window, text="Открытый ключ:", font=("Arial", 14))
        open_key_label.grid(row=1, column=0, padx=10, pady=5, sticky=tk.W)

        self.open_key_entry = tk.Entry(self.bob_window, state=tk.DISABLED, font=("Arial", 14))
        self.open_key_entry.grid(row=1, column=1, padx=10, pady=5, sticky=tk.W)

        # Поле для закрытого ключа
        private_key_label = tk.Label(self.bob_window, text="Закрытый ключ:", font=("Arial", 14))
        private_key_label.grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)

        self.private_key_entry = tk.Entry(self.bob_window, state=tk.DISABLED, font=("Arial", 14))
        self.private_key_entry.grid(row=2, column=1, padx=10, pady=5, sticky=tk.W)

        # Кнопка в окне Bob
        self.hello_button = ttk.Button(self.bob_window, text="Генерация ключей", command=self.generate_keys, style="Blue.TButton")
    ...
    def generate_keys(self):
        ...
        self.main_window.show_hello_message("Bob", open_key)
    ...
    def show(self):
        self.bob_window.deiconify()


if __name__ == "__main__":
    root = tk.Tk()
    app = BobWindow(root, None)  # Для тестирования отдельно окна Bob
    root.mainloop()
